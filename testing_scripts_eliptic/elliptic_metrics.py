...
#Compute Losses & Metrics in current epoch
        ...
        mf1 = f1_score(y, pred, average="micro", pos_label = 0)
...
#Append metrics to metrics of past epochs
        ...
        recalls_licit.append(recall_licit)

        if (epoch + 1) % 50 == 0: #Default: 50
            print(
                "Epoch: {:02d}, Train_Loss: {:.4f}, Val_Loss: {:.4f}, Precision_Illicit: {:.4f}, Recall_Illicit: {:.4f}, F1_Illicit: {:.4f}, F1_Micro: {:.4f}".format(
                    epoch + 1, train_loss, val_loss, precision_illicit, recall_illicit, if1, mf1
                )
            )
                
#Create DataFrames
loss_results = pd.DataFrame(columns = ['Iteration', 'Train_Loss', 'Val_Loss'])
metrics_results = pd.DataFrame(columns = ['Iteration', 'Precision_Illicit', 'Precision_Licit', 'Recall_Illicit', 'Recall_Licit', 'Illicit_F1', 'F1_Micro'])
...
metrics_results['F1_Micro'] = f1_micro
...
metrics_results.plot(x = 'Iteration', y = ['Precision_Illicit', 'Precision_Licit', 'Recall_Illicit', 'Recall_Licit', 'Illicit_F1', 'F1_Micro'])
